...
const getAbsoluteImageUrl = (src: string): string => {
  if (src.startsWith('http')) return src;
  // Handle imported images (which become relative paths after build)
  if (src.startsWith('/')) return `https://heavytowpro.com${src}`;
  return `https://heavytowpro.com/${src}`;
};

// Helper to format date to ISO 8601 with time component
const formatISO8601 = (dateStr: string): string => {
  // If already has time component, return as-is
  if (dateStr.includes('T')) return dateStr;
  // Add time component for proper ISO 8601
  return `${dateStr}T08:00:00-08:00`;
};

const BlogSchemas = ({ data }: BlogSchemasProps) => {
  const absoluteImageUrl = getAbsoluteImageUrl(data.heroImage.src);
  ...
  const articleSchema = {
    ...
    "datePublished": formatISO8601(data.publishDate),
    "dateModified": formatISO8601(data.dateModified || data.publishDate),
    "mainEntityOfPage": {
      "@type": "WebPage",
  ...
  return (
    <Helmet>
      ...
      <meta property="article:published_time" content={formatISO8601(data.publishDate)} />
      <meta property="article:modified_time" content={formatISO8601(data.dateModified || data.publishDate)} />
