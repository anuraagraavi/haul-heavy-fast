...
import LocationCorridorCoverage from "@/components/locations/LocationCorridorCoverage";
import { CITIES_BY_COUNTY, type LocationData, type LocationSignatureColor } from "@/data/locations";
import { landingImagesById } from "@/data/landingImages";
import { getScreenshotHubsForCountyHubPage } from "@/data/screenshotDispatchHubs";
...
const CountyTemplate = ({ data }: CountyTemplateProps) => {
  const hero = landingImagesById[data.images.hero];
  const countyCities = CITIES_BY_COUNTY[data.slug] ?? [];
  const canonicalUrl = `${BASE_URL}${data.url}`;
  const ogImageUrl = `${BASE_URL}${hero.src}`;
  const phoneHref = telHrefFromNational(data.phone);
  const countyDispatchHubs = getScreenshotHubsForCountyHubPage(data.slug);
  const multiCountyHubs = countyDispatchHubs.length > 1;
  ...
  const fireCountyHeroPhoneGtag = () => {
    if (typeof window !== "undefined") {
      window.gtag?.("event", "phone_click", {
        send_to: "AW-17927335103",
        campaign: locCampaign,
        source: "location_county_hero_phone",
      });
    }
  };

  const corridors = data.corridorCoverage ?? [];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-16">
        <section className="relative min-h-[65vh] flex items-center">
          ...
          <div className="container mx-auto px-4 relative z-10 py-12">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <p className="text-white/90 max-w-2xl mx-auto mb-6 text-lg">{data.heroSubhead}</p>
              ...
              <div className="flex flex-col gap-3 justify-center items-center">
                {multiCountyHubs ? (
                  <div className="flex flex-col gap-2 w-full max-w-md mx-auto">
                    {countyDispatchHubs.map((hub) => (
                      <Button key={hub.slug} variant="hero" size="lg" asChild className="w-full sm:w-auto">
                        <a href={telHrefFromNational(hub.phoneDisplay)} onClick={fireCountyHeroPhoneGtag}>
                          <Phone className="w-5 h-5 mr-2" />
                          Call {hub.city}: {hub.phoneDisplay}
                        </a>
                      </Button>
                    ))}
                  </div>
                  ...
                  <Button variant="hero" size="lg" asChild className="w-full sm:w-auto">
                    <a href={phoneHref} onClick={fireCountyHeroPhoneGtag}>
                      <Phone className="w-5 h-5 mr-2" />
                      Call {data.phone}
                    </a>
                  </Button>
                ...
                <Button variant="secondary" size="lg" asChild className="w-full sm:w-auto">
                  <a href="#county-quick-quote">Get a 60-Second County Quote</a>
                </Button>
              </div>
            </div>
          </div>
        ...
        <section id="county-quick-quote" className="py-16 bg-background">
          <div className="container mx-auto px-4 max-w-6xl">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
              <LocationConfidenceCard data={data} phoneHref={phoneHref} onPhoneClick={fireCountyHeroPhoneGtag} />
        ...
        <section className="py-16 bg-background">
          <div className="container mx-auto px-4 max-w-4xl">
            <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-4 text-center">Why {data.city} dispatch runs differently</h2>
        ...
        <section className="py-16 bg-muted/20">
          <div className="container mx-auto px-4">
            ...
            >
              Why fleets & drivers choose Heavy Tow Pro in {data.city}
            </h2>
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl font-bold text-foreground mb-8 text-center">County-wide service & vehicle coverage</h2>
        ...
        <LocationCorridorCoverage countyName={data.city} items={corridors} />

        <section className="py-16 bg-muted/20">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl font-bold text-foreground mb-4 text-center">Coverage: cities, freeways & common scenarios</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
              <div>
                <h3 className="text-lg font-semibold text-foreground mb-3 flex items-center gap-2 justify-center lg:justify-start">
                  <MapPin className="w-4 h-4 text-primary shrink-0" /> Cities & districts
                </h3>
              ...
              <div>
                <h3 className="text-lg font-semibold text-foreground mb-3 text-center lg:text-left">Common county scenarios</h3>
                <ul className="space-y-2">
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl font-bold text-center mb-3">Cities we serve in {data.city}</h2>
            <p className="text-muted-foreground text-center max-w-2xl mx-auto mb-10 text-sm md:text-base">
              Direct links to city pages with neighborhood-specific copy — use the carousel below for cross-county routing ideas.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {countyCities.map((city) => {
                const img = landingImagesById[city.images.hero];
                return (
                  <Link
                    key={city.slug}
                    to={city.url}
                    className="group block rounded-2xl border border-border bg-card overflow-hidden hover:shadow-elevated transition-all"
                  >
                    <div className="relative">
                      <img
                        src={img.src}
                        alt={`${city.city} towing`}
                    ...
                    <div className="p-5">
                      <h3 className="text-lg font-semibold text-foreground group-hover:text-primary transition-colors">{city.city}</h3>
                      <p className="text-sm text-muted-foreground mt-2 line-clamp-3">{city.heroSubhead}</p>
                    </div>
                  </Link>
                );
              })}
        ...
        <ProcessSteps
          ...
          phoneGtagSource="location_county_process_steps_phone"
        />
        ...
        <section className="py-16 bg-muted/20">
          <div className="container mx-auto px-4 max-w-4xl">
            <h2 className="text-3xl font-bold text-center mb-8">County FAQs — {data.city}</h2>
        ...
        <LocationsCarousel
          ...
          title="More hubs & cities we route with"
          subtitle="Hand-curated blurbs from this county hub — not a repeat of the city grid above. Same dispatch line; different geography."
        />
        <FinalCTA
          phoneDisplay={data.phone}
          phoneHref={phoneHref}
          phoneGtagCampaign={locCampaign}
          phoneGtagSource="location_county_final_cta"
        />
